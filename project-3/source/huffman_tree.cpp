...
void HuffmanTree::print() const {   // need to implement this function 
  ...
  for (char target_char : this->message) //iterate through message 
  {
    ...
    while (!st.empty())
    {
      current = st.top().first; //node
      path_to_char = st.top().second; //message so far
      st.pop(); 

      if (current == nullptr) { continue; } //skip

      if (current->character != '*') //if leaf node 
      {
        if(current->character == target_char){
          cout << path_to_char << " "; 
          break; 
        }
      }
      ...
      if (current->right != nullptr){
        st.push({current->right, path_to_char + "1"}); 
      }

      if(current->left != nullptr){ //LIFO, this will be the next node processed 
        st.push({current->left, path_to_char + "0"});
      }
    }
  }
  cout << endl; 
